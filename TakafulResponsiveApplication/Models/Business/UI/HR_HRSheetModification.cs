...
{
    public class HR_HRSheetModification
    {

        TakafulEntities tpDB = new TakafulEntities();

        public DataObjects.Internal.HR_HRSheetModification.MainObject GetInitialData()
        {

            var defaultDataObj = new DataObjects.Internal.HR_HRSheetModification.MainObject();
            ...
            {
                var sheet = new DataObjects.Internal.HR_HRSheetModification.Sheet();
        ...
        public List<DataObjects.Internal.HR_HRSheetModification.Employee> GetSheetEmployees(int sheetID)
        {

            var resultDataObj = new List<DataObjects.Internal.HR_HRSheetModification.Employee>();

            //Get employees data from the sheet data
            var sheetDataList = tpDB.HRSheetDatas
                .Where(h => h.HRS_ID == sheetID && h.HRSD_NewSheetID_Normal == null && h.HRSD_IsNewSubscription == false && h.HRSD_IsModifiedSubscription == false && h.HRSD_IsCancelledSubscription == false && h.HRSD_IsNewLoan == false && h.HRSD_IsModifiedLoan == false && h.HRSD_IsCancelledLoan == false)
                .OrderBy(h => h.Emp_ID)
                .ToList();

            for (int i = 0; i < sheetDataList.Count; i++)
            {
                var temp = new DataObjects.Internal.HR_HRSheetModification.Employee();
            ...
            using (var dbContextTransaction = tpDB.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (selectedIDs.Trim() == "")
                    {
                        ...
                        result = tpDB.Database.ExecuteSqlCommand("UPDATE HRSheetData SET HRSD_IsIncludedInNormalSheet = 'False' WHERE HRS_ID = @HRS_ID;", parameters);
                    }
                    else
                    {
                        ...
                        parameterList.Add(new SqlParameter("@HRS_ID", sheetID));
                        parameters = parameterList.ToArray();
                        result = tpDB.Database.ExecuteSqlCommand("UPDATE HRSheetData SET HRSD_IsIncludedInNormalSheet = 'True' WHERE HRSD_ID IN (" + selectedIDs + "); UPDATE HRSheetData SET HRSD_IsIncludedInNormalSheet = 'False' WHERE HRS_ID = @HRS_ID AND HRSD_ID NOT IN (" + selectedIDs + ");", parameters);
                    }
        ...
        public string ExportDataToExcel(List<List<string>> data, string path)
        {
            ...
            string fileName = "HRSheet_Normal";
