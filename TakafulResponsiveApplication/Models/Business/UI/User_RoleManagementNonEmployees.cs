...
{
    public class User_RoleManagementNonEmployees
    {

        TakafulEntities tpDB = new TakafulEntities();

        public List<DataObjects.Internal.User_RoleManagementNonEmployees.MainObject> GetInitialData()
        {

            var defaultDataObj = new List<DataObjects.Internal.User_RoleManagementNonEmployees.MainObject>();
            var bus = new Common.Common.Business();

            //Get unsubscribed employees
            ...
            {
                var temp = new DataObjects.Internal.User_RoleManagementNonEmployees.MainObject();
                ...
                if (lstEmployees[i].Emp_AccountStatus == true)
                {
                    temp.IsActive = true;
                }
        ...
        public string SetRole(long empID, int role, bool value)
        {
            ...
            else if (role == 4)  //Active
            {
                if (value == false) //Check if removing himself
                {
                    if (empID == currentEmpID)
                    {
                        return "CannotDeactivateYourself_" + empID.ToString();
                    }
                }
                employee.Emp_AccountStatus = value;
                tpDB.Entry(employee).State = EntityState.Modified;
            }
        ...
        public string ExportDataToExcel(List<List<string>> data, string path)
        {
            ...
            string fileName = "NonEmployeesRoles";
