...
namespace TakafulResponsiveApplication.Models.Business.UI
{
    public class HR_HRSheetModification_ModifiedInstallments
    {
        ...
        public DataObjects.Internal.HR_HRSheetModification_ModifiedInstallments.MainObject GetInitialData()
        {

            var defaultDataObj = new DataObjects.Internal.HR_HRSheetModification_ModifiedInstallments.MainObject();
            ...
            {
                var sheet = new DataObjects.Internal.HR_HRSheetModification_ModifiedInstallments.Sheet();
                ...
                if (lstSheets[i].HRS_IsClosedInstallmentModificationSheet == true)
                {
                    sheet.IsClosed = true;
                }
        ...
        public List<DataObjects.Internal.HR_HRSheetModification_ModifiedInstallments.Employee> GetSheetRequests(int sheetID)
        {

            var resultDataObj = new List<DataObjects.Internal.HR_HRSheetModification_ModifiedInstallments.Employee>();


            //Get ModifiedLoans requests related to this sheet
            var requestsQuery = from request in tpDB.HRSheetData_New select request;

            if (tpDB.HRSheets.First(h => h.HRS_ID == sheetID).HRS_IsClosedInstallmentModificationSheet == true)
            {
                requestsQuery = requestsQuery.Where(h => h.HRS_ID == sheetID && h.SuT_SubscriptionType == 5 && h.HRSD_IsIncluded == true);
            }
            else
            {
                requestsQuery = requestsQuery.Where(h => h.HRS_ID == sheetID && h.SuT_SubscriptionType == 5);
            }

            var lstRequests = requestsQuery.OrderByDescending(h => h.HRSD_TransactionApprovalDate).ThenBy(h => h.Emp_ID).ToList();

            //var lstRequests = tpDB.HRSheetData_New.Where(h => h.HRS_ID == sheetID && h.SuT_SubscriptionType == 5).OrderByDescending(h => h.HRSD_TransactionApprovalDate).ThenBy(h => h.Emp_ID).ToList();

            for (int i = 0; i < lstRequests.Count; i++)
            {
                var temp = new DataObjects.Internal.HR_HRSheetModification_ModifiedInstallments.Employee();
                ...
                temp.LoanInstallment = lstRequests[i].SubscriptionTransaction.SuT_Amount.Value;
        ...
        public string Save(int sheetID, string selectedIDs)
        {
            ...
            if (tpDB.HRSheets.First(s => s.HRS_ID == sheetID).HRS_IsClosedInstallmentModificationSheet == true)
            {
            ...
            using (var dbContextTransaction = tpDB.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (selectedIDs.Trim() == "")
                    {
                        ...
                        result = tpDB.Database.ExecuteSqlCommand("UPDATE HRSheetData_New SET HRSD_IsIncluded = 'False' WHERE HRS_ID = @HRS_ID AND SuT_SubscriptionType = 5;", parameters);
                    }
                    else
                    {
                        ...
                        parameterList.Add(new SqlParameter("@HRS_ID", sheetID));
                        parameters = parameterList.ToArray();
                        result = tpDB.Database.ExecuteSqlCommand("UPDATE HRSheetData_New SET HRSD_IsIncluded = 'True' WHERE HRSD_ID IN (" + selectedIDs + "); UPDATE HRSheetData_New SET HRSD_IsIncluded = 'False' WHERE HRS_ID = @HRS_ID AND HRSD_ID NOT IN (" + selectedIDs + ") AND SuT_SubscriptionType = 5;", parameters);
                    }
        ...
        public string Approve(int sheetID, string selectedIDs)
        {
            ...
            sheet.HRS_IsClosedInstallmentModificationSheet = true;
        ...
        public string ExportDataToExcel(List<List<string>> data, string path)
        {
            ...
            string fileName = "HRSheet_ModifiedInstallments";
