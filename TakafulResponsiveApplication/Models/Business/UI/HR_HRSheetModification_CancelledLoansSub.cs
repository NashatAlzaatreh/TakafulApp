...
{
    public class HR_HRSheetModification_CancelledLoansSub
    {

        TakafulEntities tpDB = new TakafulEntities();

        public DataObjects.Internal.HR_HRSheetModification_CancelledLoansSub.MainObject GetInitialData()
        {

            var defaultDataObj = new DataObjects.Internal.HR_HRSheetModification_CancelledLoansSub.MainObject();
            ...
            {
                var sheet = new DataObjects.Internal.HR_HRSheetModification_CancelledLoansSub.Sheet();
        ...
        public List<DataObjects.Internal.HR_HRSheetModification_CancelledLoansSub.Employee> GetSheetEmployees(int sheetID)
        {

            var resultDataObj = new List<DataObjects.Internal.HR_HRSheetModification_CancelledLoansSub.Employee>();

            //Get employees data from the sheet data
            var sheetDataList = tpDB.HRSheetDatas
                .Where(h => h.HRSD_NewSheetID_CL == sheetID && h.HRSD_IsCancelledLoan == true)
            ...
            {
                var temp = new DataObjects.Internal.HR_HRSheetModification_CancelledLoansSub.Employee();
                ...
                temp.IsIncluded = sheetDataList[i].HRSD_IsIncludedInCancelledLoansSheet;
        ...
        public string Save(int sheetID, string selectedIDs)
        {
            ...
            if (tpDB.HRSheets.FirstOrDefault(s => s.HRS_ID == sheetID).HRS_IsClosedCancelledLoansSheet == true)
            {
            ...
            using (var dbContextTransaction = tpDB.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (selectedIDs.Trim() == "")
                    {
                        ...
                        result = tpDB.Database.ExecuteSqlCommand("UPDATE HRSheetData SET HRSD_IsIncludedInCancelledLoansSheet = 'False' WHERE HRSD_NewSheetID_CL = @HRS_ID;", parameters);
                    }
                    else
                    {
                        ...
                        parameterList.Add(new SqlParameter("@HRS_ID", sheetID));
                        parameters = parameterList.ToArray();
                        result = tpDB.Database.ExecuteSqlCommand("UPDATE HRSheetData SET HRSD_IsIncludedInCancelledLoansSheet = 'True' WHERE HRSD_ID IN (" + selectedIDs + "); UPDATE HRSheetData SET HRSD_IsIncludedInCancelledLoansSheet = 'False' WHERE HRSD_NewSheetID_CL = @HRS_ID AND HRSD_ID NOT IN (" + selectedIDs + ");", parameters);
                    }
        ...
        public string ExportDataToExcel(List<List<string>> data, string path)
        {
            ...
            string fileName = "HRSheet_CancelledLoans_IgnoredRequests";
