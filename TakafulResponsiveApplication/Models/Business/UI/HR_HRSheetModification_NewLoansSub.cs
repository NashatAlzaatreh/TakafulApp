...
{
    public class HR_HRSheetModification_NewLoansSub
    {

        TakafulEntities tpDB = new TakafulEntities();

        public DataObjects.Internal.HR_HRSheetModification_NewLoansSub.MainObject GetInitialData()
        {

            var defaultDataObj = new DataObjects.Internal.HR_HRSheetModification_NewLoansSub.MainObject();
            ...
            {
                var sheet = new DataObjects.Internal.HR_HRSheetModification_NewLoansSub.Sheet();
        ...
        public List<DataObjects.Internal.HR_HRSheetModification_NewLoansSub.Employee> GetSheetEmployees(int sheetID)
        {

            var resultDataObj = new List<DataObjects.Internal.HR_HRSheetModification_NewLoansSub.Employee>();

            //Get employees data from the sheet data
            var sheetDataList = tpDB.HRSheetDatas
                .Where(h => h.HRSD_NewSheetID_NL == sheetID && h.HRSD_IsNewLoan == true)
            ...
            {
                var temp = new DataObjects.Internal.HR_HRSheetModification_NewLoansSub.Employee();
                ...
                temp.IsIncluded = sheetDataList[i].HRSD_IsIncludedInNewLoansSheet;
        ...
        public string Save(int sheetID, string selectedIDs)
        {
            ...
            if (tpDB.HRSheets.FirstOrDefault(s => s.HRS_ID == sheetID).HRS_IsClosedNewLoansSheet == true)
            {
            ...
            using (var dbContextTransaction = tpDB.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (selectedIDs.Trim() == "")
                    {
                        ...
                        result = tpDB.Database.ExecuteSqlCommand("UPDATE HRSheetData SET HRSD_IsIncludedInNewLoansSheet = 'False' WHERE HRSD_NewSheetID_NL = @HRS_ID;", parameters);
                    }
                    else
                    {
                        ...
                        parameterList.Add(new SqlParameter("@HRS_ID", sheetID));
                        parameters = parameterList.ToArray();
                        result = tpDB.Database.ExecuteSqlCommand("UPDATE HRSheetData SET HRSD_IsIncludedInNewLoansSheet = 'True' WHERE HRSD_ID IN (" + selectedIDs + "); UPDATE HRSheetData SET HRSD_IsIncludedInNewLoansSheet = 'False' WHERE HRSD_NewSheetID_NL = @HRS_ID AND HRSD_ID NOT IN (" + selectedIDs + ");", parameters);
                    }
        ...
        public string ExportDataToExcel(List<List<string>> data, string path)
        {
            ...
            string fileName = "HRSheet_NewLoans_IgnoredRequests";
