...
{
    public class HR_HRSheetModification_NewSubscriptionsSub
    {

        TakafulEntities tpDB = new TakafulEntities();

        public DataObjects.Internal.HR_HRSheetModification_NewSubscriptionsSub.MainObject GetInitialData()
        {

            var defaultDataObj = new DataObjects.Internal.HR_HRSheetModification_NewSubscriptionsSub.MainObject();
            ...
            {
                var sheet = new DataObjects.Internal.HR_HRSheetModification_NewSubscriptionsSub.Sheet();
        ...
        public List<DataObjects.Internal.HR_HRSheetModification_NewSubscriptionsSub.Employee> GetSheetEmployees(int sheetID)
        {

            var resultDataObj = new List<DataObjects.Internal.HR_HRSheetModification_NewSubscriptionsSub.Employee>();

            //Get employees data from the sheet data
            var sheetDataList = tpDB.HRSheetDatas
                .Where(h => h.HRSD_NewSheetID_NS == sheetID && h.HRSD_IsNewSubscription == true)
            ...
            {
                var temp = new DataObjects.Internal.HR_HRSheetModification_NewSubscriptionsSub.Employee();
                ...
                temp.IsIncluded = sheetDataList[i].HRSD_IsIncludedInNewSubsSheet;
        ...
        public string Save(int sheetID, string selectedIDs)
        {
            ...
            if (tpDB.HRSheets.FirstOrDefault(s => s.HRS_ID == sheetID).HRS_IsClosedNewSubsSheet == true)
            {
            ...
            using (var dbContextTransaction = tpDB.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (selectedIDs.Trim() == "")
                    {
                        ...
                        result = tpDB.Database.ExecuteSqlCommand("UPDATE HRSheetData SET HRSD_IsIncludedInNewSubsSheet = 'False' WHERE HRSD_NewSheetID_NS = @HRS_ID;", parameters);
                    }
                    else
                    {
                        ...
                        parameterList.Add(new SqlParameter("@HRS_ID", sheetID));
                        parameters = parameterList.ToArray();
                        result = tpDB.Database.ExecuteSqlCommand("UPDATE HRSheetData SET HRSD_IsIncludedInNewSubsSheet = 'True' WHERE HRSD_ID IN (" + selectedIDs + "); UPDATE HRSheetData SET HRSD_IsIncludedInNewSubsSheet = 'False' WHERE HRSD_NewSheetID_NS = @HRS_ID AND HRSD_ID NOT IN (" + selectedIDs + ");", parameters);
                    }
        ...
        public string ExportDataToExcel(List<List<string>> data, string path)
        {
            ...
            string fileName = "HRSheet_NewSubscriptions_IgnoredRequests";
