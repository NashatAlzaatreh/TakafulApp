...
{
    public class Request_CommitteeEditSubscription_Approve
    {

        TakafulEntities tpDB = new TakafulEntities();

        public DataObjects.Internal.Request_CommitteeEditSubscription_Approve.MainObject GetInitialData(long empID, int year, int serial)
        {

            var defaultDataObj = new DataObjects.Internal.Request_CommitteeEditSubscription_Approve.MainObject();
            ...
            if (employee != null)
            {
                ...
                var request = tpDB.SubscriptionTransactions.FirstOrDefault(s => s.Emp_ID == empID && s.SuT_Year == year && s.SuT_Serial == serial && s.SuT_SubscriptionType == 2 && (s.SuT_ApprovalStatus == 4 || s.SuT_ApprovalStatus == 5));
        ...
        public string TakeAction(long empID, int year, int serial, int action)
        {
            ...
            TransactionCommitteeDecision tcd = tpDB.TransactionCommitteeDecisions.FirstOrDefault(t => t.Emp_ID == empID && t.SuT_Year == year && t.SuT_Serial == serial && t.SuT_SubscriptionType == 2 && t.TCD_CommitteeMemberID == committeeMemberID);
        ...
        public DataObjects.Internal.Request_CommitteeEditSubscription_Approve.NavigationObject Navigate(long empID, int year, int serial, int direction)
        {

            var defaultDataObj = new DataObjects.Internal.Request_CommitteeEditSubscription_Approve.NavigationObject();
