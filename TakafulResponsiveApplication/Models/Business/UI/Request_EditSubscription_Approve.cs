...
namespace TakafulResponsiveApplication.Models.Business.UI
{
    public class Request_EditSubscription_Approve
    {
        ...
        public DataObjects.Internal.Request_EditSubscription_Approve.MainObject GetInitialData(long empID, int year, int serial)
        {

            var defaultDataObj = new DataObjects.Internal.Request_EditSubscription_Approve.MainObject();
            ...
            if (employee != null)
            {
                ...
                var request = tpDB.SubscriptionTransactions.FirstOrDefault(s => s.Emp_ID == empID && s.SuT_Year == year && s.SuT_Serial == serial && s.SuT_SubscriptionType == 2 && (s.SuT_ApprovalStatus == 1 || s.SuT_ApprovalStatus == 4));
                ...
                defaultDataObj.NewSubscriptionAmount = request.SuT_Amount;
        ...
        public DataObjects.Internal.Request_EditSubscription_Approve.NavigationObject Navigate(long empID, int year, int serial, int direction)
        {

            var defaultDataObj = new DataObjects.Internal.Request_EditSubscription_Approve.NavigationObject();
            var bus = new Common.Common.Business();
            ...
            if (direction == 1)
            {
                lst = tpDB.SubscriptionTransactions.Where(s => s.SuT_SubscriptionType == 2 && (s.SuT_ApprovalStatus == 1 || s.SuT_ApprovalStatus == 4) && (s.SuT_Year > year || s.SuT_Serial > serial)).OrderBy(s => s.SortIndex).ToList();
            }
            else if (direction == 2)
            {
                lst = tpDB.SubscriptionTransactions.Where(s => s.SuT_SubscriptionType == 2 && (s.SuT_ApprovalStatus == 1 || s.SuT_ApprovalStatus == 4) && (s.SuT_Year < year || s.SuT_Serial < serial)).OrderByDescending(s => s.SortIndex).ToList();
            }
            else
            {
                return null;
        ...
        public string TakeAction(long empID, int year, int serial, string notes, int action)
        {
            ...
            {
                case 1: //Approve
                    result = bus.TakeActionToEditSubscriptionRequest(empID, year, serial, notes, Common.Common.Business.RequestAction.Approve, adminID);
                    if (result == "True")
                    {
                ...
                case 2: //Transfer to committee
                    result = bus.TakeActionToEditSubscriptionRequest(empID, year, serial, notes, Common.Common.Business.RequestAction.TransferToCommittee, null);
                    if (result == "True")
                    {
                        return "Transferred";
                    }
                    break;
                case 3: //Reject
                    result = bus.TakeActionToEditSubscriptionRequest(empID, year, serial, notes, Common.Common.Business.RequestAction.Reject, adminID);
                    if (result == "True")
                    {
                        //Send notification email to the employee
                        if (!string.IsNullOrEmpty(emp.Emp_Email))
