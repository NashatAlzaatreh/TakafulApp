...
{
    public class HR_HRSheetModification_ModifiedSubscriptions
    {

        TakafulEntities tpDB = new TakafulEntities();

        public DataObjects.Internal.HR_HRSheetModification_ModifiedSubscriptions.MainObject GetInitialData()
        {

            var defaultDataObj = new DataObjects.Internal.HR_HRSheetModification_ModifiedSubscriptions.MainObject();
            ...
            {
                var sheet = new DataObjects.Internal.HR_HRSheetModification_ModifiedSubscriptions.Sheet();
        ...
        public List<DataObjects.Internal.HR_HRSheetModification_ModifiedSubscriptions.Employee> GetSheetRequests(int sheetID)
        {

            var resultDataObj = new List<DataObjects.Internal.HR_HRSheetModification_ModifiedSubscriptions.Employee>();
            var bus = new Common.Common.Business();


            //Get ModifiedSubs requests related to this sheet
            var requestsQuery = from request in tpDB.HRSheetData_New select request;

            if (tpDB.HRSheets.First(h => h.HRS_ID == sheetID).HRS_IsClosedModificationSheet == true)
            {
                requestsQuery = requestsQuery.Where(h => h.HRS_ID == sheetID && h.SuT_SubscriptionType == 2 && h.HRSD_IsIncluded == true);
            }
            else
            {
                requestsQuery = requestsQuery.Where(h => h.HRS_ID == sheetID && h.SuT_SubscriptionType == 2);
            }

            var lstRequests = requestsQuery.OrderByDescending(h => h.HRSD_TransactionApprovalDate).ThenBy(h => h.Emp_ID).ToList();

            //var lstRequests = tpDB.HRSheetData_New.Where(h => h.HRS_ID == sheetID && h.SuT_SubscriptionType == 2).OrderByDescending(h => h.HRSD_TransactionApprovalDate).ThenBy(h => h.Emp_ID).ToList();

            for (int i = 0; i < lstRequests.Count; i++)
            {
                var temp = new DataObjects.Internal.HR_HRSheetModification_ModifiedSubscriptions.Employee();
        ...
        public string Save(int sheetID, string selectedIDs)
        {
            ...
            if (tpDB.HRSheets.First(s => s.HRS_ID == sheetID).HRS_IsClosedModificationSheet == true)
            {
            ...
            using (var dbContextTransaction = tpDB.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (selectedIDs.Trim() == "")
                    {
                        ...
                        result = tpDB.Database.ExecuteSqlCommand("UPDATE HRSheetData_New SET HRSD_IsIncluded = 'False' WHERE HRS_ID = @HRS_ID AND SuT_SubscriptionType = 2;", parameters);
                    }
                    else
                    {
                        ...
                        parameterList.Add(new SqlParameter("@HRS_ID", sheetID));
                        parameters = parameterList.ToArray();
                        result = tpDB.Database.ExecuteSqlCommand("UPDATE HRSheetData_New SET HRSD_IsIncluded = 'True' WHERE HRSD_ID IN (" + selectedIDs + "); UPDATE HRSheetData_New SET HRSD_IsIncluded = 'False' WHERE HRS_ID = @HRS_ID AND HRSD_ID NOT IN (" + selectedIDs + ") AND SuT_SubscriptionType = 2;", parameters);
                    }
        ...
        public string ExportDataToExcel(List<List<string>> data, string path)
        {
            ...
            string fileName = "HRSheet_ModifiedSubscriptions";
