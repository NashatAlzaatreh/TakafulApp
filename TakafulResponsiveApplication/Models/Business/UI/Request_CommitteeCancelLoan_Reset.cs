...
{
    public class Request_CommitteeCancelLoan_Reset
    {

        TakafulEntities tpDB = new TakafulEntities();

        public DataObjects.Internal.Request_CommitteeCancelLoan_Reset.MainObject GetInitialData(long empID, int year, int serial)
        {

            var defaultDataObj = new DataObjects.Internal.Request_CommitteeCancelLoan_Reset.MainObject();
        ...
        public string Reset(long empID, int year, int serial)
        {
            ...
            {
                Context.Database.ExecuteSqlCommand("DELETE FROM TransactionCommitteeDecision WHERE Emp_ID = {0} AND SuT_Year = {1} AND SuT_Serial = {2} AND SuT_SubscriptionType = {3}; ", new object[] { empID, year, serial, 6 });
            }
