...
{
    public class Employee_UploadPaidSubscriptions
    {

        TakafulEntities tpDB = new TakafulEntities();

        public List<DataObjects.Internal.Employee_UploadPaidSubscriptions.Employee> GetUploadedData(string filePath, out string resultMessage)
        {

            var defaultDataObj = new List<DataObjects.Internal.Employee_UploadPaidSubscriptions.Employee>();
            ...
            if (Path.GetExtension(filePath) != ".xls" && Path.GetExtension(filePath) != ".xlsx")
            {
                ...
                return new List<DataObjects.Internal.Employee_UploadPaidSubscriptions.Employee>();
            }
            ...
            var lstEmployees = tpDB.Employees
                ...
                .Where(em => arrIDs.Contains(em.Emp_ID) && em.Emp_AccountStatus == true && em.FundSubscriptions.FirstOrDefault(fs => fs.FSu_Status == 1) != null)
                .OrderBy(e => e.Emp_ID).ToList();

            for (int i = 0; i < lstEmployeesIDs.Count; i++)
            {
                var temp = new DataObjects.Internal.Employee_UploadPaidSubscriptions.Employee();
                ...
                int fsID = employee.FundSubscriptions.FirstOrDefault(fs => fs.FSu_Status == 1).FSu_ID;
        ...
        public object Save(List<DataObjects.External.Employee_UploadPaidSubscriptions_In> employeesData)
        {
        ...
        public string ExportDataToExcel(List<List<string>> data, string path)
        {
            ...
            string fileName = "UploadedPaidSubscriptions";
